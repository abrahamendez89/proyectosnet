...
exports.consultaEncuestas = function(req, res){
  mySQL.queryMySQL('select i.id_encuesta, i.titulo as tituloEncuesta, i.descripcion, ie.id_opcion, ie.titulo as tituloOpcion,'+
  ...
  function(encuestasOpciones){
    if(encuestasOpciones.length > 0)
    {
      ...
      ,function(result){
        ...
        res.json({"exito": false, "mensaje": "Error al obtener los datos de encuesta por usuario." });
      });

      
    }
    else
      res.json({"exito": false, "mensaje": "No existen encuestas activas." });
  }, function(error){
  ...
  mySQL.conecta(function(connection){
    ...
    , function(result){
      ...
      if(result[0].votos == 0){
        ...
        res.json({"exito": true, "mensaje": "Un integrante de tu casa ya votó en esta encuesta, recuerda que solo está permitido un voto por casa."});
      }
...
exports.detallesEncuesta = function(req, res){
  ...
  function(result){
    if(result.length > 0){
      ...
      res.json({"exito": false, "mensaje": "Aún no hay personas que hayan votado."});
    }
  }, function(error){
    es.json({"exito": false, "mensaje": "Error al consultar los detalles de la encuesta.", "obj": error});
  });
};