...
namespace Herramientas.ORM
{
    public class ObjetoBase : IDisposable
    {
        ...
        public iSQL SQL { get { return sql; } set { sql = value; } }
        public ManejadorDB Manejador { get { return manejador; } set { manejador = value; } }
        private List<String> AtributosCargadosPorPropiedad = new List<string>();
        private List<String> AtributosCargadosPorPropiedadCopia = new List<string>();
                ...
                if (AtributosCargadosPorPropiedad.Count > 0)
                {
                    ...
                    foreach (String atributo in temp)
                    {
                        ...
                        if (atrib.FieldType.Name.Contains("List"))//si entra entonces es una lista relacionada y se tiene q considerar
                        {
                            ...
                            sql.EjecutarConsulta(eliminacionConsulta, new List<object>() { id });
                            ...
                            if (listaObjetos != null)
                            {
                                foreach (ObjetoBase objBaseLista in listaObjetos)
                                {
                                    ...
                                    sql.EjecutarConsulta(consultaAgregadoRelacionado, new List<object>() { id, idTempLista, DateTime.Now });
        ...
        private Boolean BorrarObjetoPermanentemente()
        {
            try
            {
                ...
                try
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        ...
                        if (nombreTabla.ToUpper().Contains("_X__"))
                        {
                            ...
                            sql.EjecutarConsulta(queryDelete, new List<object>() { id });
                        }
                        else
                        {
                    ...
                    sql.EjecutarConsulta(queryBorrarObjeto, new List<object>() { id });
                }
                catch (Exception ex)
                {
        ...
        public T GetAtributoRelacionado<T>(String nombreObjeto)
        {
            try
            {
        ...
        public List<TipoObjectoLista> GetListaRelacionados<TipoObjectoLista>(String nombreAtributoLista)
        {
            try
            {
                FieldInfo atributoRel = this.GetType().GetField(nombreAtributoLista, BindingFlags.Instance | BindingFlags.NonPublic);
        ...
        #endregion
        #region Agregar y/o quitar objetos de la lista relacionada de forma directa
        //agregar un elemento a lista
        public void BorrarObjetosDeListaRelacionadaPermanentemente<TipoLista>(String nombreLista)
        {
            Type tipo = this.GetType();
            String nombreTablaRelacion = tipo.Name + "_X_" + typeof(TipoLista).Name + "_" + nombreLista;

            String queryids = "select _contenido from " + nombreTablaRelacion + " where _contenedor = " + id;
            DataTable ids = manejador.EjecutarConsulta(queryids);
            ...
            if (ids.Rows.Count > 0)
            {
                ...
                {
                    idsEliminar += dr["_contenido"] + ",";
                }
                idsEliminar = idsEliminar.Substring(0, idsEliminar.Length - 1);

                String queryDeleteTablaRelacion = "delete from " + nombreTablaRelacion + " where _contenedor = " + id;
                manejador.EjecutarConsulta(queryDeleteTablaRelacion);
                String queryEliminarObjetosLista = "delete from " + typeof(TipoLista).Name + " where id in (" + idsEliminar + ")";
                manejador.EjecutarConsulta(queryEliminarObjetosLista);
            }
        }
        public void AgregadoRapidoAListaRelacionada(ObjetoBase objeto, String nombreLista)
        {
            try
            {
                ...
                sql.EjecutarConsulta(query, new List<object>() { id, idObjetoLista });
            }
            catch (Exception ex)
            {
                throw new Exception(ex.Message);
        ...
        public void CopiarA<T>(T objetoDestino)
        {
            Type tipo = this.GetType();
            FieldInfo[] atributos = tipo.GetFields(System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
            ...
            {
                Object value = atributo.GetValue(this);
                atributo.SetValue(objetoDestino, value);
            }
            //return (T)copia;
        }
        #endregion
        #region manejo de formato json
        public String ConvertirAJson(int niveles)
        {
            return Herramientas.Web.JSON.ConvertirObjetoBaseAJSON(this, niveles);
        }
        public String ConvertirAJson()
        {
            return Herramientas.Web.JSON.ConvertirObjetoBaseAJSON(this);
        }
        public static T ConvertirDeJson<T>(String json)
        {
            return Herramientas.Web.JSON.ConvertirJSONAObjeto<T>(json);
        }
        #endregion
