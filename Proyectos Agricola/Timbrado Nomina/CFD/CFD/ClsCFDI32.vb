...
#Region "Métodos Públicos"
    ...
    Public Function TimbresLibres(ByVal prmRFCEmisor As Integer) As Integer
        ...
        Try
            vcSQL = "SELECT * FROM EMPRESAS..FAC_EMISORES(NOLOCK) WHERE NRFCEMISOR = " & prmRFCEmisor
    ...
    Public Function TimbresLibres(ByVal prmRFCEmisor As String) As Integer
        ...
        Try
            vcSQL = "select par.CVALORPAR From empresas..FAC_EMISORES emi inner join empresas..FAC_PARAMETROS par on emi.nRFCEmisor = par.NRFCEMISOR where emi.cRFC = '" & prmRFCEmisor & "' and CCLAVEPAR = 'USUARIOWS'"
            DAO.RegresaConsultaSQL(vcSQL, DT)
            If Not DT Is Nothing AndAlso DT.Rows.Count > 0 Then
                vcUsuario = DT.Rows(0)(0).ToString()
            End If

            vcSQL = "select par.CVALORPAR From empresas..FAC_EMISORES emi inner join empresas..FAC_PARAMETROS par on emi.nRFCEmisor = par.NRFCEMISOR where emi.cRFC = '" & prmRFCEmisor & "' and CCLAVEPAR = 'PASSWORDWS'"
            DAO.RegresaConsultaSQL(vcSQL, DT)
            If Not DT Is Nothing AndAlso DT.Rows.Count > 0 Then
                vcPassword = DT.Rows(0)(0).ToString()
            End If
    ...
    Public Function GeneraFacturaElectronica(ByVal prmID As Long, ByVal nRFCEmisor As Int32, ByVal prmEsFacturaNomina As Boolean) As String
        Try
            ...
            vcRutaEstiloCadenaOriginal32 = vObjCabecera.RutaUtileria
            ...
            'actualizando la fecha de la bd a la fecha Actual

            Dim dbAcceso As New Herramientas.SQL.SQLServer()
            dbAcceso.ConfigurarConexion(DAO.GetNombreServidor(), DAO.GetNombreBaseDeDatos(), DAO.GetLoginUsuario(), DAO.GetPassUsuario())



            Dim dtFecha As DataTable = dbAcceso.EjecutarConsulta("select getdate()")

            Dim parametros As New List(Of Object)
            parametros.Add(Convert.ToDateTime(dtFecha.Rows(0)(0)))
            Dim nFacturaFecha As String = vObjCabecera.nFactura.ToString()
            parametros.Add(prmID)

            dbAcceso.EjecutarConsulta("update f set f.dfecha = @dfecha from empresas..FAC_ENCFACTURASCFDINOM f where nFactura = @nFactura", parametros)

            cfd.fecha = Convert.ToDateTime(parametros(0).ToString()) 'Convert.ToDateTime(Convert.ToDateTime(parametros(0).ToString()).ToString("yyyy-MM-ddThh:mm:ss"))
            ...
            Catch ex As Exception
                Throw New Exception(ex.Message & ": " & vObjCabecera.RutaCert)
            End Try
            Dim i As Integer
            ...
            vcXMLTemporal = vObjCabecera.RutaXml & "\" & vObjCabecera.RFC & "_" & vObjCabecera.EFSERIE.Trim & vObjCabecera.nFactura & "_temp.xml"
            vcXMLTemporal2 = vObjCabecera.RutaXml & "\" & vObjCabecera.RFC & "_" & vObjCabecera.EFSERIE.Trim & vObjCabecera.nFactura & "_temp2.xml"
            ...
            ' AQUI INICIA EL COMPLEMENTO DE NOMINA
            If (prmEsFacturaNomina) Then
                ...
                If Not DTCabeceroNomina.Rows(0)("FECHAPAGO") Is DBNull.Value Then
                ...
                If Not DTCabeceroNomina.Rows(0)("FECHAINICIALPAGO") Is DBNull.Value Then
                ...
                If Not DTCabeceroNomina.Rows(0)("FECHAFINALPAGO") Is DBNull.Value Then
                ...
                If Not DTCabeceroNomina.Rows(0)("FECHAINICIORELLABORAL") Is DBNull.Value Then
                    AttNomina = CFDI.CreateAttribute("FechaInicioRelLaboral")
                    AttNomina.Value = DTCabeceroNomina.Rows(0)("FECHAINICIORELLABORAL").ToString("yyyy-MM-dd")
                    Nomina.Attributes.Append(AttNomina)
                End If
                ...
                If Not DTCabeceroNomina.Rows(0)("PeridiocidadPago") Is DBNull.Value AndAlso DTCabeceroNomina.Rows(0)("PeridiocidadPago") <> "" Then
                    AttNomina = CFDI.CreateAttribute("PeriodicidadPago")
                    AttNomina.Value = DTCabeceroNomina.Rows(0)("PeridiocidadPago")
                    Nomina.Attributes.Append(AttNomina)
                End If
                ...
                For Each vRowPercep As DataRow In DTPercepciones.Rows
                    ...
                    vnTotalExentoPercepciones += vRowPercep("NIMPORTEEXENTO")
                    vnTotalGravadoPercepciones += vRowPercep("NIMPORTEGRAVADO")
                ...
                For Each vRowDeduc As DataRow In DTDeducciones.Rows
                    ...
                    vnTotalExentoDeducciones += vRowDeduc("NIMPORTEEXENTO")
                    vnTotalGravadoDeducciones += vRowDeduc("NIMPORTEGRAVADO")
            ...
            ' AQUI TERMINA EL COMPLEMENTO DE NOMINA


            If File.Exists(vObjCabecera.RutaXml & "\" & vObjCabecera.RFC & "_" & vObjCabecera.EFSERIE.Trim & vObjCabecera.nFactura & ".xml") Then
                Kill(vObjCabecera.RutaXml & "\" & vObjCabecera.RFC & "_" & vObjCabecera.EFSERIE.Trim & vObjCabecera.nFactura & ".xml")
            End If

            CFDI.Save(vObjCabecera.RutaXml & "\" & vObjCabecera.RFC & "_" & vObjCabecera.EFSERIE.Trim & vObjCabecera.nFactura & ".xml")

            vcNombreID = vObjCabecera.RFC & "_" & vObjCabecera.EFSERIE.Trim & vObjCabecera.nFactura.Trim

            Dim vObjTimbra As New ClsFD

            vObjTimbra.Connect(atrConexion)

            Dim xmlContenido As String = CFDI.InnerText

            Dim rutaXMLSinTimbrar As String = vObjCabecera.RutaXml & "\" & vObjCabecera.RFC & "_" & vObjCabecera.EFSERIE.Trim & vObjCabecera.nFactura & ".xml"

            vcTimbrado = vObjTimbra.Timbra(rutaXMLSinTimbrar, prmID, vObjCabecera.DestinoXml, _
                              vObjCabecera.DestinoAcuseXml, vObjCabecera.RutaBidimensional, vObjCabecera.UsuarioWS, _
                              vObjCabecera.PasswordWS, vObjCabecera.RFC, vObjCabecera.CLIENTERFC, vObjCabecera.EFIMPORTE, cadenaOriginal, vcNombreID, nRFCEmisor)

            If vcTimbrado.Trim().Equals("") Then
                'si no hay errores se agrega el registro.
                Dim xmlTimbrado As String = vObjCabecera.DestinoXml & "\" & vcNombreID & ".xml"

                'recuperando informacion de timbrado
                Dim objReaderTemp As New StreamReader(xmlTimbrado)
                Dim ContenidoXML As String = ""
                Dim sLine As String = ""
                Do
                    sLine = objReaderTemp.ReadLine()
                    If Not sLine Is Nothing Then
                        ContenidoXML += sLine & vbCrLf
                    End If
                Loop Until sLine Is Nothing
                objReaderTemp.Close()

                '-------------------------------------------------------------------------------------------------------------
                'aqui tenemos recuperado el xml con el timbrado realizado y lo guardamos en la BD

                Dim nFacturaXML As String = prmID.ToString().Substring(0, prmID.ToString().Length)
                Dim nRFCEmisorXML As Int32 = prmID.ToString().Substring(prmID.ToString().Length - 1, 1)

                Dim dtEmisor As DataTable = dbAcceso.EjecutarConsulta("select nRFCEmisor from empresas..fac_emisores where cRFC = '" & vObjCabecera.RFC & "'")
                nRFCEmisorXML = Convert.ToInt32(dtEmisor(0)(0))

                Dim nRFCEmpleadoXML As String = vObjCabecera.CLIENTERFC

                Dim TemporadaXML As String = nFacturaXML.Substring(0, 2) & "-" & nFacturaXML.Substring(2, 2)
                Dim NominaXML As String = nFacturaXML.Substring(4, 2)
                Dim SemanaXML As String = nFacturaXML.Substring(6, 2)
                Dim EmpleadoXML As String = nFacturaXML.Substring(8, 5)
                Dim importeXML As Double = vObjCabecera.EFIMPORTE
                Dim fechaTimbradoXML As DateTime = vObjCabecera.EFFECHA
                ...
                Try
                    ...
                    valores.Add(nFacturaXML)
                    valores.Add(nRFCEmisorXML)
                    valores.Add(nRFCEmpleadoXML)
                    valores.Add(TemporadaXML)
                    valores.Add(NominaXML)
                    valores.Add(SemanaXML)
                    valores.Add(EmpleadoXML)
                    valores.Add(fechaTimbradoXML)
                    valores.Add(importeXML)
                    valores.Add(ContenidoXML)
    ...
    Public Function CancelaFacturaElectronica(ByVal prmID As String) As String()
        ...
        Try
            CancelaFacturaElectronica = vObjTimbra.Cancela(prmID)
        Catch ex As Exception
            CancelaFacturaElectronica = {ex.Message, ""}
        End Try
