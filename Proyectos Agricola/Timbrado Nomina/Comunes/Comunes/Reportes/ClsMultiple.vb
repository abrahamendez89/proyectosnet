Public Class ClsMultiple
    ...
    Private atrCodigoIncorrecto As Boolean
    ...
    Public Property Value() As String
        Get
            Dim ret As String = AccTextBoxAdvanced1.SeleccionParaString.Replace("'", "")
            If ret.Trim = "*" Then
                Return ""
            Else
                Return ret
    ...
    Public Function SeleccionParaString() As String
        Return AccTextBoxAdvanced1.SeleccionParaString()
    End Function
    ...
    Private Sub ClsMultiple_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
    ...
    Public Function ValidayObtenDescripcion() As Boolean
        If InStr(AccTextBoxAdvanced1.Text, ",") > 0 Then
            Return True
        Else
    ...
    Public Sub ConfiguraFiltrosPermisos(ByRef prmRow As DataRow, ByRef prmDTPermisos As DataTable, ByVal prmValidating As Boolean, ByVal prmSugerirDefault As Boolean)
        ...
        Comunes.Comun.Permisos.ClsManejadorPermisos.ConfiguraFiltrosPermisosReporteador(AccTextBoxAdvanced1, prmRow, prmDTPermisos, prmValidating, prmSugerirDefault)
    End Sub

    Private Sub ClsMultiple_Validating(ByVal sender As Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles Me.Validating
        If CType(sender, ClsMultiple).Metacatalogo = "" Then Return

        If CType(sender, ClsMultiple).Value <> "" And CType(sender, ClsMultiple).Label2.Text = "" Then
            ...
            CType(sender, ClsMultiple).Value = ""
            CType(sender, ClsMultiple).CodigoIncorrecto = True
            e.Cancel = True
            Exit Sub
        End If

        If CType(sender, ClsMultiple).RowFiltro Is Nothing Then Exit Sub

        If Comunes.Comun.ClsTools.IfNull(CType(sender, ClsMultiple).RowFiltro("bEsPermiso"), False) Then
            'Es permiso principal, obtenemos todos `los catàlogos a los que tiene que filtrar
            For Each dr As DataRow In atrFiltrosGlobalReporte.Select("nTipoPermiso = " & CType(sender, ClsMultiple).RowFiltro("nTipoPermiso"))
    ...
    Public Sub ValidaElementoSecundarioPermisos()
        ...
        End If
        vcWHERE &= " AND " & Me.AccTextBoxAdvanced1.CatalogoBase.MetaCatalogoBase.CampoPrimario & " IN (" & Me.AccTextBoxAdvanced1.Text.Trim & ")"
        vcSQL &= vcWHERE
