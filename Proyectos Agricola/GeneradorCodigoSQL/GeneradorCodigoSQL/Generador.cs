...
namespace GeneradorCodigoSQL
{
    public partial class Generador : Form
    {
        ...
        private void CargarCache()
        {
            ...
            txt_serverMysql.Text = variables.ObtenerValorVariable<String>("MYSQL_SERVIDOR");
            //txt_instancia.Text = variables.ObtenerValorVariable<String>("INSTANCIA");
            txt_bdMysql.Text = variables.ObtenerValorVariable<String>("MYSQL_BD");
            txt_usuarioMysql.Text = variables.ObtenerValorVariable<String>("MYSQL_USUARIO");
            txt_contraMysql.Text = variables.ObtenerValorVariable<String>("MYSQL_CONTRASEÑA");
        ...
        private Boolean CargarDll()
        {
            ...
            {
                Herramientas.Forms.Mensajes.Error("No se pudo carcar la DLL: " + txt_rutaDLL.Text);
                return false;
            }
                    ...
                    for (int i = 0; i < source.Columns.Count; i++)
                    {
                        ...
                        else if (source.Columns[i].DataType == typeof(Bitmap))
                        {
                            try
                            {
                                ...
                                Bitmap imagen = (Bitmap)Imagenes.BitArrayTOBitmap((byte[])resultado.Rows[j][i]);
        ...
        private void dgb_resultados_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                Bitmap imagen = (Bitmap)Imagenes.BitArrayTOBitmap((byte[])resultado.Rows[e.RowIndex][e.ColumnIndex]);
        ...
        private void btn_conectarMysql_Click(object sender, EventArgs e)
        {
            baseDat = txt_bdMysql.Text.Trim();
            if (esNueva)
                baseDat = "default";

            String cadena = "SERVER = @SERVIDOR; DATABASE = @BASEDATOS; UID = @USUARIO; PASSWORD = @CONTRASENA";
            if (esNueva)
            {
                cadena = cadena.Replace("DATABASE = @BASEDATOS;", "");
            }
            cadena = cadena.Replace("@SERVIDOR", txt_serverMysql.Text.Trim()).Replace("@BASEDATOS", baseDat).Replace("@USUARIO", txt_usuarioMysql.Text.Trim()).Replace("@CONTRASENA", txt_contraMysql.Text.Trim());
            try
            {
                sql = new MySQL(cadena);
            }
            catch (Exception ex)
            {
                txt_EstatusMysql.BackColor = Color.Red;
                txt_EstatusMysql.Text = "Error";
                MessageBox.Show("Ocurrio un error: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                grb_consultasRapidas.Enabled = false;
                if (MessageBox.Show("Si la causa del error se debe a que la base de datos '" + txt_bdMysql.Text.Trim() + "' es nueva, puede crearla de forma completa. ¿Desea realizar el proceso de creación?", "Checar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                {
                    esNueva = true;
                    txt_bdMysql.Enabled = false;
                    NombreBD = txt_bdMysql.Text.Trim();
                    ...
                    btn_conectarMysql_Click(null, null);

                }
            }
            txt_EstatusMysql.BackColor = Color.Green;
            txt_EstatusMysql.Text = "Conectado";
            temp = txt_EstatusMysql;

            CargarDatosBD();

            variables.GuardarValorVariable("MYSQL_SERVIDOR", txt_serverMysql.Text);
            //variables.GuardarValorVariable("INSTANCIA", txt_instancia.Text);
            variables.GuardarValorVariable("MYSQL_BD", txt_bdMysql.Text);
            variables.GuardarValorVariable("MYSQL_USUARIO", txt_usuarioMysql.Text);
            variables.GuardarValorVariable("MYSQL_CONTRASEÑA", txt_contraMysql.Text);
            variables.ActualizarArchivo();
        }
    }
}
